...
-- Example: Trainers can read bookings for slots assigned to them
...
BEGIN
  ...
  ELSIF (TG_OP = 'UPDATE') THEN
    ...
    IF OLD.status != 'CANCELLED' AND NEW.status = 'CANCELLED' THEN
      UPDATE slots SET current_bookings = current_bookings - 1 WHERE id = OLD.slot_id AND current_bookings > 0;
    -- If status changed from CANCELLED to CONFIRMED
    ELSIF OLD.status = 'CANCELLED' AND NEW.status != 'CANCELLED' THEN
      UPDATE slots SET current_bookings = current_bookings + 1 WHERE id = NEW.slot_id AND current_bookings < max_capacity;
      IF NOT FOUND THEN
        RAISE EXCEPTION 'Slot % is already fully booked', NEW.slot_id;
      END IF;
